// https://guoxue.httpcn.com/book/daxue/
// https://guoxue.httpcn.com/book/zhongyong/
(async function () {
  ...
  // 选择所有的含有章节的 div 元素
  const sections = document.querySelectorAll(".lunyu_section.mengzi_section");
  ...
  for (const section of sections) {
    ...
    for (const li of listItems) {
      ...
      links.forEach((link) => {
        // 将章节信息推入章节数组
        book.chapters.push({
          title: link.innerText, // 章节标题
          link: link.href.startsWith("//") ? "https:" + link.href : link.href, // 完整 URL
        });
      });
    }
